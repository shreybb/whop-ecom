...
/** Whop webhook payloads may use company/account, account_id, or company_id depending on API version. */
export async function resolveCatalogCompanyId(
	...
	try {
		...
		const accountId =
			(plan as { account?: { id?: string }; company?: { id?: string } }).account
				?.id ??
			(plan as { company?: { id?: string } }).company?.id;
		if (accountId) return accountId;
		const productId = plan.product?.id;
		if (!productId) return null;
		const product = await getWhopSdk().products.retrieve(productId);
		return (
			(product as { account?: { id?: string }; company?: { id?: string } })
				.account?.id ??
			(product as { company?: { id?: string } }).company?.id ??
			null
		);
	} catch (err) {
